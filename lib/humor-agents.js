...
class Agent {
    ...
    async generateAdvice(context, lessonsLearned = [], searchResults = []) {
        let learningContext = "";
...
`;
        ...
        if (searchResults.length > 0) {
            searchContext = `
お笑い芸人やプロのファシリテーターのテクニック:
${searchResults.map(s => `- ${s.title}: ${s.snippet}`).join('\n')}

上記のテクニックを参考に、具体的なアドバイスを生成してください。
`;
        }
...
Based on the meeting context and search results, provide CONCRETE ADVICE for the presenter on:
...
class HumorOrchestrator {
    ...
    async intervene(moodContext) {
        // Step 0: Search for comedian/presenter techniques
        const searchResults = await searchWeb("お笑い芸人 話の振り方 場の空気 盛り上げ方 ファシリテーション テクニック");
        ...
        // Step 3: Selected agent provides advice (with search results)
        const advice = await selectedAgent.generateAdvice(moodContext, lessonsLearned, searchResults);
