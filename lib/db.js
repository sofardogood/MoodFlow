...
async function getVisualData(sessionId) {
    try {
        const [moods, transcripts, overrides, questions, highlights, session] = await Promise.all([
            prisma.mood.findMany({
                where: { sessionId },
        ...
        return {
            isActive: session ? session.isActive : true, // Default to true if not found to avoid premature redirect
